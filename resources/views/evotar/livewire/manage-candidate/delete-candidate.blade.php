<div x-data="{ open: false }" x-cloak @candidate-deleted.window="open = false">
    <!-- Trigger Button -->
        ...
        >
            ...
            <div class="flex justify-between items-center mb-2">
                <div class="flex items-center">
                    ...
                    <h2 class="text-sm font-semibold text-red-600">Delete Candidate</h2>
                </div>
                <!-- Close Button (X) -->
            ...
            @if($this->election->status == 'ongoing' || $this->election->status == 'completed')
                    ...
                    <div class="flex">
                        ...
                        <div class="ml-3">
                            <p class="text-sm text-yellow-700">
                                Candidates cannot be deleted while the election is {{ $this->election->status }}.
                            </p>
                        </div>
                    </div>
                </div>
            @else
                <form action="" wire:submit.prevent="deleteCandidate">
                    <div>
                        <p class="text-gray-700 mb-6 ml-5 text-xs text-left">Are you sure you want to remove this candidate? This process cannot be undone.</p>

                        <div class="flex justify-end mt-4 space-x-2">
                            <button @click="open = false" type="button"
                                    class="px-4 py-2 border border-gray-300 text-gray-800 rounded-md shadow-md hover:bg-gray-400 text-xs">
                                Cancel
                            </button>
                            <button type="submit"
                                    class="px-4 py-2 bg-red-600 text-white rounded-md shadow-md hover:bg-red-700 text-xs">
                                Delete
                            </button>
                        </div>
                    </div>
