                    ...
                    <div class="flex flex-col md:flex-row w-full items-center justify-between">
                        <div class="flex items-center justify-between flex-wrap md:flex-nowrap gap-2">
                            <button
                                ...
                                wire:click="exportPrograms"
                                wire:loading.attr="disabled">
                                <svg wire:loading.remove wire:target="exportPrograms" xmlns="http://www.w3.org/2000/svg" height="20px" viewBox="0 -960 960 960"
                                ...
                                <span wire:loading.remove wire:target="exportPrograms" class="text-[12px]">Export List of Programs</span>
                                <svg wire:loading wire:target="exportPrograms" class="animate-spin h-5 w-5 mr-3" viewBox="0 0 24 24">
                                    <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor"
                                            stroke-width="4"></circle>
                                    <path class="opacity-75" fill="currentColor"
                                          d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
                                </svg>
                                <span wire:loading wire:target="exportPrograms">Exporting...</span>
                            </button>
                        </div>
                    ...
                    <div class="mt-4 min-h-[400px]">
                        <div class="space-y-6">
                            @foreach($colleges as $college)
                                <div x-data="{ open: false }" class="bg-white shadow-lg rounded-lg p-4" wire:key="campus-{{ $college->id }}">
                                    <!-- College Header -->
                                    <div class="flex justify-between items-center">
                                        <h2 class="text-[14px] font-bold">{{ $college->name }}</h2>

                                        <!-- Hide Edit Button if No Programs -->
                                            <div class="flex">
                                                <div>
                                                    <livewire:manage-program.add-program :collegeId="$college->id" :key="'add-program-'.$college->id" />
                                                </div>
                                    ...
                                    <div class="mt-3 text-[12px]">
                                        @if($college->programs->count() > 0)
                                            @foreach($college->programs as $program)
                                                <div class="bg-gray-100 p-3 rounded mt-2 flex justify-between items-center" wire:key="program-{{ $program->id }}">
                                                    <span>{{ $program->name }}</span>
                                                    <!-- Action Buttons (Hidden by Default, Shown When Campus Edit is Clicked) -->
                                                    <div x-show="open" class="flex space-x-2" x-cloak>
                                                        <livewire:manage-program.edit-program :programId="$program->id" :key="'edit-program-'.$program->id"/>
                                                        <livewire:manage-program.delete-program :programId="$program->id" :key="'delete-program-'.$program->id"/>
                                                    </div>
                                                </div>
                                            @endforeach
                                        @else
                                            <!-- Message when no colleges exist -->
                                            <p class="text-gray-500 italic text-center mt-2">No currently added programs for this college</p>
                                        @endif
                                    </div>
                                </div>
                            @endforeach
                        ...
                        <!-- Pagination -->
                        <div class="mt-4">
                            {{ $colleges->links('evotar.components.pagination.tailwind-pagination') }}
                        </div>
                    </div>
                </div>
            </div>
