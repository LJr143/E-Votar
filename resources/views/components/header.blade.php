...
<style>
    #profileDropdown {
        ...
        position: absolute; /* Ensure it positions relative to the header or parent */
    }

    .header-container {
        overflow: visible; /* Prevent clipping of the dropdown */
    }
    ...
    <div class="flex w-1/2">
        ...
        <div class="flex justify-end relative w-1/2 px-4">
            <div class="border-[1px] border-gray-800 rounded-full">
                <img class="h-[35px] rounded-full" src="{{ asset('storage/assets/profile/cat_meme.jpg') }}"
                     alt="usep_logo">
            </div>
            <div class="ml-2">
                <h3 class="text-gray-900 text-[12px] tracking-tight uppercase font-semibold">{{auth()->user()->first_name}} {{auth()->user()->middle_initial}}
                    . {{auth()->user()->last_name}}</h3>
                <p class="text-gray-500 text-[10px] capitalize">{{ auth()->user()->getRoleNames()->join(', ') }}</p>
            </div>
            <div class="flex justify-center items-center px-4 cursor-pointer" onclick="toggleProfileDropdown(event)">
                <svg width="11" height="5" viewBox="0 0 12 6" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path
                        d="M11.3137 0.155061C11.1257 -0.0328917 10.8316 -0.0499784 10.6244 0.103801L10.565 0.155061L5.99816 4.72165L1.43134 0.155062C1.24338 -0.0328913 0.949267 -0.0499779 0.742012 0.103802L0.682634 0.155062C0.494681 0.343014 0.477595 0.63713 0.631374 0.844385L0.682634 0.903763L5.62381 5.84494C5.81176 6.03289 6.10588 6.04998 6.31313 5.8962L6.37251 5.84494L11.3137 0.903762C11.5204 0.697014 11.5204 0.361809 11.3137 0.155061Z"
            ...
            <!-- Profile Dropdown -->
            <div class="absolute right-0 mt-20 w-[210px] bg-white rounded-md shadow-lg z-20 hidden" id="profileDropdown">
                <ul class="py-1">
                    <!-- Account Settings -->
                    <li class="flex text-[12px] items-center px-7 py-3 hover:bg-gray-200 cursor-pointer">
...
<script>
    // Dropdown toggle for Profile dropdown
