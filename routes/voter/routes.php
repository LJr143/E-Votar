...
Route::middleware(['splash.screen', 'check.deactivated', 'single.voter.session', 'check.blocked.ip', 'track.ip.user', 'facial.verified'])->prefix('voter')->group(function () {
    ...
    Route::get('/voting-process/{slug}', [VoterElectionController::class, 'voting'])->name('voter.voting')->middleware(['voter.auth' , 'verified.voter', 'vote.checker', 'election.status']);
    Route::get('/voting/vote-submission', [VoterElectionController::class, 'confirmVoting'])->name('voter.voting.confirm')->middleware('voter.auth', 'vote.checker', 'verified.voter' );
